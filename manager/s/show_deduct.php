...
if(!intval($in['ID']))
{
	...
	$dinfo = $db->get_row("SELECT * FROM ".DATATABLE."_order_deduct where CompanyID = ".$_SESSION['uinfo']['ucompany']." and DeductID=".intval($in['ID'])." and DeductUser=".$_SESSION['uinfo']['userid']." limit 0,1");
	if(empty($dinfo['OrderID'])) exit('数据不存在!');

	$listinfo = $db->get_results("SELECT c.ContentID,c.ContentName,c.ContentColor,c.ContentSpecification,d.ProductDeduct,d.ProductTotal,d.DeductTotal FROM ".DATATABLE."_order_deduct_cart d left join ".DATATABLE."_order_cart c ON d.CartID=c.ID where d.CompanyID=".$_SESSION['uinfo']['ucompany']." and d.OrderID=".$dinfo['OrderID']." and c.OrderID=".$dinfo['OrderID']." ");
}
?>
