...
?>
                <tr id="line_<? echo $lsv['ID'];?>" class="bottomline" onmouseover="inStyle(this)"  onmouseout="outStyle(this)">
				  <td >&nbsp;<? echo $n++;?></td>
                    <?php if($in['showImg'] != 'F') { ?>
                    <td>
                        <img src="<?php echo RESOURCE_URL . $lsv['Picture']; ?>" alt="<?php echo $lsv['Name']; ?>" style="height:80px;max-width:110px;" />

                    </td>
                        <td title="包装：<? echo $lsv['Casing'];?>"><a href="product_content.php?ID=<? echo $lsv['ID'];?>" target="_blank">
                                <? echo ($lsv['Coding'] ? $lsv['Coding'] . '<br/>' : '') . $lsv['Name'];?>
                            </a></td>
                    <?php } else { ?>
			...
			echo $lsv[$cinfo['ClientSetPrice']]." / ".$lsv['Units'];
                  ...
                  <?php
                    ...
                    <?php
                        ...
                        <td align="center"><a onclick="javascript:addtocart('<? echo $lsv['ID'];?>','<? echo $oinfo ['OrderID'];?>','<? echo $cinfo['ClientID'];?>');" href="javascript:void(0);" title="包装：<? echo $lsv['Casing'];?>"><strong>&#8250; 添加到订单</strong></a></td>
                    <?php
                    }
                  ?>
                  <!--<td align="center"><a onclick="javascript:addtocart('<?/* echo $lsv['ID'];*/?>','<?/* echo $oinfo ['OrderID'];*/?>','<?/* echo $cinfo['ClientID'];*/?>');" href="javascript:void(0);" title="包装：<?/* echo $lsv['Casing'];*/?>"><strong>&#8250; 添加到订单</strong></a></td>-->
