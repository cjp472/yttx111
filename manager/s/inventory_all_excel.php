...
$odata = $db->get_results("SELECT i.ID,i.SiteID,i.Name,i.Coding,i.Units,i.Casing,i.Color,i.Specification,n.OrderNumber,n.ContentNumber FROM ".DATATABLE."_order_content_index i left join ".DATATABLE."_order_number n on i.ID=n.ContentID where i.CompanyID = ".$_SESSION['uinfo']['ucompany']." and i.FlagID=0 ORDER BY i.OrderID DESC, i.ID DESC ".$endsql);
