...
class XmlData
{
	...
	if(is_array($dataarr))
	{
		foreach($dataarr as $dvar)
		{
			$dvar['ODate'] = substr($dvar['ODate'],1);
			$dkey = substr($dvar['ODate'],4,2)."-".substr($dvar['ODate'],6,2);
			//$dvararr[$dkey] = $dvar;
			$msg1 .='<string>'.$dkey.'</string>
			';
	...
	<draw>
		<text shadow="high" color="cc0000" font="黑体" alpha="90" size="20" x="8" y="6" width="180" height="50" h_align="center">退单统计</text>
	</draw>
	<chart_label shadow="low" alpha="65" size="10" position="inside" as_percentage="true" />
	<chart_pref select="true" drag="false" rotation_x="50" />
	<chart_rect x="65" y="50" width="650" height="180" positive_alpha="0" />
	...
	{
		if (!@file_exists ("../resource/".$_SESSION['uinfo']['ucompany']."/xml/"))
		{
			$this->_mkdir("../resource/".$_SESSION['uinfo']['ucompany']."/xml/");
		}
		
		$handle = fopen("../resource/".$_SESSION['uinfo']['ucompany']."/xml/".$key.".xml", "w");
		@flock($handle, 3);
