...
//总条数
	...
	function multi($num, $perpage, $curpage, $mpurl, $maxpages = 0) 
	{
		...
		$multipage = ($curpage - $offset > 1 && $pages > $page ? '<a href="'.$mpurl.'page=1">&nbsp;首页&nbsp;' : '').($curpage > 1 ? '<a style="color:#33a676" class="changeb" href="'.$mpurl.'page='.($curpage - 1).'"><</a>' : '');
		for($i = $from; $i <= $to; $i++) {
			$multipage .= $i == $curpage ? '<span class="page_bar_current">'.$i.'</span>' : '<a href="'.$mpurl.'page='.$i.'">'.$i.'</a>';
		}

// 		$multipage .= ($curpage < $pages ? '<a style="color:#33a676" class="changeb" href="'.$mpurl.'page='.($curpage + 1).'">></a>' : '').($curpage + $page - $offset <= $pages ? '<a href="'.$mpurl.'page='.$pages.'">&nbsp;尾页&nbsp;</a>' : '').($curpage == $maxpages ? '<a href="'.$mpurl.'pages='.$maxpages.'">&gt;</a>' : '').($pages > $page ? '
//		' : '');
		$multipage .= ($curpage < $pages ? '<a href="'.$mpurl.'page='.($curpage + 1).'">></a>' : '').($curpage + $page - $offset <= $pages ? '<a href="'.$mpurl.'page='.$pages.'">&nbsp;尾页&nbsp;</a>' : '').($curpage == $maxpages ? '<a href="'.$mpurl.'pages='.$maxpages.'">&gt;</a>' : '').($pages > $page ? '<input type="text" name="custompage" onKeypress="if ((event.keyCode < 48 || event.keyCode > 57) && event.keyCode != 13) event.returnValue = false;"  onKeyDown="javascript: if(event.keyCode == 13) window.location=\''.$mpurl.'page=\'+this.value;">' : '');
		...
		$multipage = $multipage ? '<div class="page_bar">'.''.'<span class="">&nbsp;&nbsp;共&nbsp;'.$num.'&nbsp;条&nbsp;/&nbsp;'.$realpages.'&nbsp;页&nbsp;</span>'.$multipage.''.'</div>' : '';
		}
	return $multipage;
	}
...
//生成导航条 1
        ...
        {
        	return $this->multi($this->Total, $this->PageSize, $this->PageNum(), $pageurl.'?rt=etong'.$this->Url($this->LinkAry), $maxpages = 0);
        }
        function ShowLink2($pageurl='') 
        {
        	return $this->multi2($this->Total, $this->PageSize, $this->PageNum(), $pageurl.'?rt=etong'.$this->Url($this->LinkAry), $maxpages = 0);
        }
 }
?>