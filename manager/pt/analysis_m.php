...
<ul>
	...
	<li>- <a href="analysis_m.php"  class="locationli">月交易数据</a></li>
</ul>
...
<!-- tree -->   
		  ...
		  <div class="line" >
			...
			<legend>月交易数据分析</legend>

<?php
	$sqlmsg = '';
	if(empty($in['y']))	$year = date("Y"); else $year = $in['y'];
	foreach($databasearr as $v)
	{
		if(empty($v)) $dname = DB_DATABASE."."; else $dname = DB_DATABASE."_".$v.".";
		
		$yearsql  = "SELECT left(OrderSN,6) as ODate,sum(OrderTotal) as totalmoney,count(*) as totalnumber from ".$dname.DATATABLE."_order_orderinfo where left(OrderSN,4)='".$year."' AND OrderStatus != 8 
  AND OrderStatus != 9 and OrderTotal < 1000000 and OrderCompany > 20 group by ODate ";
...
?>
                 <table width="100%" border="0" cellspacing="0" cellpadding="0" >
     			 <tr>
					 ...
					 <td height="30" >&nbsp;请选择年份:&nbsp;<select name="y" id="y" >
						<?php
						$by = 2011;
						...
						?>
					</select>&nbsp;年 &nbsp;<input type="submit" name="newbutton" id="newbutton" value=" 查看 " class="mainbtn" /></td>
					</form>
     			 </tr>
			</table>
			...
			foreach($ydata as $k=>$v){
				$carr[] = "'".str_replace($year,'',$k)."'";
				$tarr[] = $v['number'];
				$narr[] = $v['money'];
			?>
			 <tr  >
                  <td align="center" height="50" width="20%" class="sfont18"><?php echo str_replace($year,'',$k); ?> 月</td>
