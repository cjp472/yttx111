...
*/
    ...
    if(!empty($in['bdate'])) $sqlmsg .= "  and c.ClientDate >= '".strtotime($in['bdate'] . '00:00:00')."' ";
    if(!empty($in['edate'])) $sqlmsg .= "  and c.ClientDate <= '".strtotime($in['edate'] . '23:59:59')."' ";
...
$InfoDataNum = $db->get_row("SELECT count(*) AS allrow FROM ".DATATABLE."_order_client AS c LEFT JOIN ".DATABASEU.DATATABLE."_three_sides_merchant AS m ON c.ClientCompany=m.CompanyID AND c.ClientID=m.MerchantID left join ".DATABASEU.DATATABLE."_credit_main as main on  c.ClientCompany = main.CompanyID AND c.ClientID = main.ClientID where 1=1 ".$sqlmsg);// AND c.ClientID=m.MerchantID 
$page = new ShowPage;
$page->PageSize = 50;
...
//读取信息
$ucSql = "SELECT c.*,m.*,main.CreditStatus FROM ".DATATABLE."_order_client AS c LEFT JOIN ".DATABASEU.DATATABLE."_three_sides_merchant AS m ON c.ClientCompany = m.CompanyID AND c.ClientID = m.MerchantID  left join ".DATABASEU.DATATABLE."_credit_main as main on  c.ClientCompany = main.CompanyID AND c.ClientID = main.ClientID where 1=1  ".$sqlmsg." ORDER BY CONVERT(main.CreditStatus USING gbk) DESC ";
    ...
    <div id="bodycontent">
    	...
    	<div id="searchline">
        	...
        	<div class="leftdiv">
        	  ...
        	  <form id="FormSearch" name="FormSearch" method="get" action="credit_client.php">
			...
			<div class="location"><strong>当前位置：</strong><a href="credit_client.php">账期审核</a> </div>
          ...
          <form id="MainForm" name="MainForm" method="post" action="" target="exe_iframe" >
              ...
              <table width="100%" border="0" cellspacing="0" cellpadding="0">
     			 <tr>
   			       ...
   			       <td  class="sublink"></td>
       			     <td width="50%" align="right"><? echo $page->ShowLink('credit_client.php');?></td>
     			 </tr>
              </table>
			  <table width="100%" border="0" cellspacing="0" cellpadding="0">
 	...
 	function ShowTreeMenuList($resultdata,$p_id) 
	{
		...
		foreach($resultdata as $key => $var)
		{
			if($var['AreaParent'] == $p_id)
			{
				if($var['AreaParent'] == "0")
				{
					$frontMsg  .= '<li><a href="credit_client.php?aid='.$var['AreaID'].'"  ><strong>'.$var['AreaName'].'</strong></a>';
				}	
				else
				{
					$frontMsg  .= '<li><a href="credit_client.php?aid='.$var['AreaID'].'"  >'.$var['AreaName'].'</a>';
				}	
