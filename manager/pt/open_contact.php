...
$company_sql = "SELECT c.*,s.* FROM ".DATABASEU.DATATABLE."_order_company c left join ".DATABASEU.DATATABLE."_order_cs s on c.CompanyID=s.CS_Company where c.CompanyID={$company_id}  ORDER BY c.CompanyID DESC";
$company_info = $db->get_row($company_sql);
if($in['m'] == 'record_info') {
    //记录信息
    $cuser = $_SESSION['uinfo']['usertruename'];
    $user_name = $_SESSION['uinfo']['username'];
    $content = $in['content'];
    $count = $db->get_var("SELECT COUNT(*) AS Total FROM ".DATABASEU.DATATABLE."_buy_account_log WHERE CompanyID={$company_id} LIMIT 1");
    $rst = $db->query("INSERT INTO ".DATABASEU.DATATABLE."_buy_account_log (CompanyID,CreateDate,CreateUser,Content) VALUES ({$company_id},".time().",'{$cuser}','{$content}')");
    if($rst) {
        if((int)$count == 0) {
            $db->query("UPDATE ".DATABASEU.DATATABLE."_buy_account SET AdminUser='{$user_name}' WHERE company_id={$company_id} LIMIT 1");
        }
        exit('ok');
    } else {
        exit('记录信息发生错误,请重试!' . $db->last_query);
    }
    exit;
}
...
<div id="bodycontent">
    ...
    <div class="bline">
        ...
        <?php
        $sql = "SELECT * FROM ".DATABASEU.DATATABLE."_buy_account_log WHERE CompanyID=" . $in['id'] . " ORDER BY CreateDate ASC";
        ...
        <form id="MainForm" name="MainForm" method="post" action="" target="exe_iframe" >
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
                ...
                if(!empty($list_data))
                {
                    ...
                    <? } }else{?>
                    <tr>
                        <td colspan="5" height="30" align="center">暂无该用户的回访联系信息!</td>
                    </tr>
                <? }?>
                </tbody>
            </table>
        ...
        <div style="border-top:2px solid #ccc;">
            <form id="MainForm_Back"  method="post" action="open_contact.php" target="exe_iframe" >
                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tbody>
                    <tr>
                        ...
                        <td><?php echo $company_info['CompanyName']; ?></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td align="center" style="line-height:40px;">
                        ...
                        <td><?php echo $company_info['CompanyContact']; ?></td>
                        <td></td>
                    </tr>
                    <tr>
                        <td align="center" style="line-height:40px;">联系电话：</td>
                        <td><?php echo $company_info['CompanyMobile']; ?></td>
                        <td></td>
                    </tr>
                    <tr id="line_<? echo $lsv['ID'];?>" class="bottomline" onmouseover="inStyle(this)"  onmouseout="outStyle(this)"  >
                        ...
                        <td>
                            ...
                            <input name="id" value="<?php echo $company_info['CompanyID'] ?>" type="hidden"/>
