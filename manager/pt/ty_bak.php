...
<div id="bodycontent">
    ...
    <div class="bline">
        ...
        $InfoDataNum = $db->get_results("SELECT IsReg,VisitType,COUNT(*) as Total FROM (
                                            SELECT o.VisitType,o.IsReg FROM ".DATABASEU.DATATABLE."_experience_contact o where 1=1 ".$sqlmsg.$groupmsg." ORDER BY o.id DESC".
            ")EC GROUP BY IsReg,VisitType");
        foreach($InfoDataNum as $calc)
        {
            $CNT[$calc['IsReg']][$calc['VisitType']] = $calc['Total'];
        }
        
        $allrow = (empty(array_sum($CNT['F'])) ? 0 : array_sum($CNT['F'])) + (empty(array_sum($CNT['T'])) ? 0 : array_sum($CNT['T']));
        ...
        $datasql   = "SELECT * FROM (
                            SELECT o.* FROM ".DATABASEU.DATATABLE."_experience_contact o where 1=1 ".$sqlmsg.$groupmsg." ORDER BY o.id DESC".
                     ")EC";
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
     			 <tr>
       				 <td class="bottomlinebold">
                         ...
                         <label>未注册体验用户&nbsp;&nbsp;<?php echo empty(array_sum($CNT['F'])) ? '0' : array_sum($CNT['F'])?>&nbsp;人，&nbsp;</label>
                         <label>
                                                                                     其中电脑端&nbsp;&nbsp;<?php echo empty($CNT['F']['PC']) ? '0' : $CNT['F']['PC']; ?>&nbsp;人，
                                                                                     移动端&nbsp;&nbsp;<?php echo empty($CNT['F']['MOBILE']) ? '0' : $CNT['F']['MOBILE']; ?>人； &nbsp;
                         </label>
                         <label>已注册体验用户&nbsp;&nbsp;<?php echo empty(array_sum($CNT['T'])) ? '0' : array_sum($CNT['T']) ?>&nbsp;人，&nbsp;</label>
                         <label>
                                                                                     其中电脑端&nbsp;&nbsp;<?php echo empty($CNT['T']['PC']) ? '0' : $CNT['T']['PC']; ?>&nbsp;人，
                                                                                     移动端&nbsp;&nbsp;<?php echo empty($CNT['T']['MOBILE']) ? '0' : $CNT['T']['MOBILE']; ?>人 &nbsp;
                         </label>
       				 </td>
