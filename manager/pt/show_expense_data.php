...
<script type="text/javascript">
	...
	function delete_data(did)
	{
		if(confirm('确认删除吗?'))
		{
			...
			$.post("do_delete.php",
				{m:"delete_expense", cid: cid, did:did},
				function(data){
					if(data == "ok"){
	...
	function delete_mul_data()
	{
		if(confirm('确认删除吗?'))
		{
			var cid = "<?php echo $cid;?>";
			$.blockUI({ message: "<p>正在执行，请稍后...</p>" }); 
			$.post("do_delete.php?m=delete_expense", $("#MainForm").serialize(),
				function(data){
					if(data == "ok"){
						$.blockUI({ message: "<p>删除成功!</p>" });
...
<body>
			<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center" class="tc" >
        	  <form id="FormSearch" name="FormSearch" method="get" action="show_expense_data.php">		  
				...
				<tr>
					...
					<td width="250" nowrap="nowrap"><input type="text" name="begindate" id="begindate" class="inputline" style="width:100px;" value="<? if(!empty($in['begindate'])) echo $in['begindate'];?>" /> - <input type="text" name="enddate" id="enddate" class="inputline" style="width:100px;" value="<? if(!empty($in['enddate'])) echo $in['enddate'];?>" /></td>
       				<td width="60"><input name="searchbutton" type="submit" class="redbtn" id="searchbutton" value="搜 索" /> </td>
					<td align="right"><input name="expensebutton" type="button" class="bluebtn" id="expensebutton" value="收款单" onclick="window.location.href='show_finance_data.php?cid=<?php echo $cid;?>'" /></td>
				</tr>
				</thead>
   	          </form>
			 </table> 
			  ...
			  <table width="100%" border="0" cellspacing="0" cellpadding="0">
               <thead>
                <tr>
				  ...
				  <td width="14%" class="bottomlinebold">类型</td>
...
<?php	
	...
	{
		$sqlmsg .= " and ExpenseDate >= '".$in['begindate']."' ";
		$pagearr['begindate'] = $in['begindate'];
	}
	if(!empty($in['enddate']))
	{
		$sqlmsg .= " and ExpenseDate <= '".$in['enddate']."' ";
		$pagearr['enddate'] = $in['enddate'];
	}
	$sqlnum = "SELECT count(*) AS allrow FROM ".$sdbname.DATATABLE."_order_expense where CompanyID = ".$cid." ".$sqlmsg." ";
	$datasql   = "SELECT * FROM ".$sdbname.DATATABLE."_order_expense where CompanyID = ".$cid." ".$sqlmsg." Order by ExpenseID Desc";
	...
	if(!empty($list_data))
	{
		...
		$billdata = $db->get_results("select BillID,BillNO,BillName from ".$sdbname.DATATABLE."_order_expense_bill where CompanyID=".$cid." ");
...
?>
                <tr id="line_<? echo $lsv['ExpenseID'];?>" class="bottomline" onmouseover="inStyle(this)"  onmouseout="outStyle(this)" >
                  <td >&nbsp;<span class="selectinput"><input type="checkbox" name="selectedID[]" id="select_<? echo $lsv['ExpenseID'];?>" value="<? echo $lsv['ExpenseID'];?>" /></span>&nbsp;<? echo $n++;?></td>
				  <td height="28" >
					<span ><? echo $lsv['ExpenseID'];?></span>
				  </td>
				  <td><?php echo $billarr[$lsv['BillID']];?></td>
                  <td ><?php echo $clientarr[$lsv['ClientID']];?></td>
                  <td >
				  <?php 
				  echo "<span title='金额' class=font12>¥ ".$lsv['ExpenseTotal']."</span> ";
				  ?>
					</td>
				  <td><?php echo $lsv['ExpenseDate'];?></td>
                  <td >
					<? echo $finance_arr[$lsv['FinanceFlag']];?>			
				  </td>
				  <td>[<a href="javascript:void(0)" onclick="delete_data('<?php echo $lsv['ExpenseID'];?>');">删除</a>]</td>
                </tr>
	<? } }else{?>
     			 <tr>
	...
	<? }?>
              ...
              <table width="100%" border="0" cellspacing="0" cellpadding="0">
     			 <tr>
				   ...
				   <td  align="right"><? echo $page->ShowLink('show_expense_data.php');?></td>
     			 </tr>
              </table>
