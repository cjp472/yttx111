...
<script type="text/javascript">
	...
	function delete_data(did)
	{
		if(confirm('确认删除吗?'))
		{
			...
			$.post("do_delete.php",
				{m:"delete_consignment", cid: cid, did:did},
				function(data){
					if(data == "ok"){
	...
	function delete_mul_data()
	{
		if(confirm('确认删除吗?'))
		{
			var cid = "<?php echo $cid;?>";
			$.blockUI({ message: "<p>正在执行，请稍后...</p>" }); 
			$.post("do_delete.php?m=delete_consignment", $("#MainForm").serialize(),
				function(data){
					if(data == "ok"){
						$.blockUI({ message: "<p>删除成功!</p>" });
...
<body>
			<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center" class="tc" >
        	  <form id="FormSearch" name="FormSearch" method="get" action="show_consignment_data.php">			  
        		<input type="hidden" name="cid" id="cid2"  value="<? if(!empty($cid)) echo $cid;?>"   />
				<thead>
				<tr>
					<td width="80" align="center"><strong>发货单搜索：</strong></td> 
        	        ...
        	        <select name="ostatus" id="ostatus" class="selectline">
						<option value="" >发货单状态</option>
						<?php
						foreach($incept_arr as $key=>$var)
						{
			  ...
			  <table width="100%" border="0" cellspacing="0" cellpadding="0">
               <thead>
                <tr>
                  ...
                  <td width="18%" class="bottomlinebold" >运单号</td>
				  <td width="10%" class="bottomlinebold" >状态</td>
                  <td width="8%" class="bottomlinebold" nowrap="nowrap">&nbsp;管理</td>
                </tr>
     		 </thead>      		
      		<tbody >
<?php	
	...
	if(!empty($in['kw'])){	
		...
		if($in['stype']=='idsn'){
			...
			$sqlmsg .=  " AND ConsignmentID IN(".$in['kw'].")";
		}else{
			$in['kw'] = str_replace(",","','",$in['kw']);
			$sqlmsg .=  " AND ConsignmentOrder IN('".$in['kw']."')";
		}
	}
	if(isset($in['ostatus']) and $in['ostatus'] != ''){
		$sqlmsg .= ' and ConsignmentFlag = '.$in['ostatus'].' ';
		$pagearr['ostatus'] = $in['ostatus'];
	}

	if(!empty($in['begindate']))
	{
		$sqlmsg .= ' and ConsignmentDate > '.strtotime($in['begindate'].'00:00:00').' ';
		$pagearr['begindate'] = $in['begindate'];
	}
	if(!empty($in['enddate']))
	{
		$sqlmsg .= ' and ConsignmentDate < '.strtotime($in['enddate'].'23:59:59').' ';
		$pagearr['enddate'] = $in['enddate'];
	}

	$sqlnum = "SELECT count(*) AS allrow FROM ".$sdbname.DATATABLE."_order_consignment where ConsignmentCompany = ".$cid." ".$sqlmsg." ";
	$datasql   = "SELECT ConsignmentID,ConsignmentClient,ConsignmentOrder,ConsignmentLogistics,ConsignmentNO,ConsignmentMan,ConsignmentDate,ConsignmentFlag,InceptCompany FROM ".$sdbname.DATATABLE."_order_consignment where ConsignmentCompany = ".$cid." ".$sqlmsg." Order by ConsignmentID Desc";
	...
	if(!empty($list_data))
	{
		...
		{
?>
                <tr id="line_<? echo $lsv['ConsignmentID'];?>" class="bottomline" onmouseover="inStyle(this)"  onmouseout="outStyle(this)" >
                  <td >&nbsp;<span class="selectinput"><input type="checkbox" name="selectedID[]" id="select_<? echo $lsv['ConsignmentID'];?>" value="<? echo $lsv['ConsignmentID'];?>" /></span>&nbsp;<? echo $n++;?></td>
				  <td height="28" >
					<span ><? echo $lsv['ConsignmentID'];?></span>
				  </td>
				  <td><? echo $lsv['ConsignmentOrder'];?></td>
                  <td >		
						<span><? echo $lsv['InceptCompany'];?></span>
				  </td>

                  <td >
				  <?php 
				  echo "<span class=font12> ".$lsv['ConsignmentNO']."</span> ";
				  ?>
					</td>
                  <td >
					<? echo $incept_arr[$lsv['ConsignmentFlag']];?>			
				  </td>
				  <td>[<a href="javascript:void(0)" onclick="delete_data('<?php echo $lsv['ConsignmentID'];?>');">删除</a>]</td>
                </tr>
	<? } }else{?>
     			 <tr>
	...
	<? }?>
              ...
              <table width="100%" border="0" cellspacing="0" cellpadding="0">
     			 <tr>
				   ...
				   <td  align="right"><? echo $page->ShowLink('show_consignment_data.php');?></td>
     			 </tr>
              </table>
