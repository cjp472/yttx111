...
<script type="text/javascript">
	...
	function delete_data(did)
	{
		if(confirm('确认删除吗?'))
		{
			...
			$.post("do_delete.php",
				{m:"delete_library", cid: cid, did:did},
				function(data){
					if(data == "ok"){
	...
	function delete_mul_data()
	{
		if(confirm('确认删除吗?'))
		{
			var cid = "<?php echo $cid;?>";
			$.blockUI({ message: "<p>正在执行，请稍后...</p>" }); 
			$.post("do_delete.php?m=delete_library", $("#MainForm").serialize(),
				function(data){
					if(data == "ok"){
						$.blockUI({ message: "<p>删除成功!</p>" });
	...
	function clear_library()
	{
		if(confirm('确认清空所有库存吗?'))
		{
			...
			$.blockUI({ message: "<p>正在执行，请稍后...</p>" }); 
			$.post("do_delete.php",
				{m:"clear_library", cid: cid},
				function(data){
					if(data == "ok"){
						$.blockUI({ message: "<p>清空成功!</p>" }); 
					}else{
						$.blockUI({ message: "<p>"+data+"</p>" }); 
					}					
				}		
...
<body>
			<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center" class="tc" >
        	  <form id="FormSearch" name="FormSearch" method="get" action="show_library_data.php">			  
        		<input type="hidden" name="cid" id="cid2"  value="<? if(!empty($cid)) echo $cid;?>"   />
				<thead>
				<tr>
					<td width="80" align="center"><strong>入库单搜索：</strong></td> 
        	        ...
        	        <select name="stype" id="stype" class="selectline">
						<option value="ordersn" <?php if($in['stype']=="ordersn") echo 'selected="selected"'; ?> >单号</option>
					...
					<td align="right"><input name="clearbutton" type="button" class="bluebtn" id="clearbutton" value="清空库存" onclick="clear_library();" /></td>
				</tr>
				</thead>
   	          </form>
			 </table> 
			  ...
			  <table width="100%" border="0" cellspacing="0" cellpadding="0">
               <thead>
                <tr>
				  ...
				  <td width="20%" class="bottomlinebold">单号</td>
                  <td  class="bottomlinebold">经办人</td>
...
<?php	
	...
	if(!empty($in['kw'])){	
		...
		if($in['stype']=='idsn'){
			...
			$sqlmsg .=  " AND StorageID IN(".$in['kw'].")";
		}else{
			$in['kw'] = str_replace(",","','",$in['kw']);
			$sqlmsg .=  " AND StorageSN IN('".$in['kw']."')";
		}
	}
	if(!empty($in['begindate']))
	{
		$sqlmsg .= ' and StorageDate > '.strtotime($in['begindate'].'00:00:00').' ';
		$pagearr['begindate'] = $in['begindate'];
	}
	if(!empty($in['enddate']))
	{
		$sqlmsg .= ' and StorageDate < '.strtotime($in['enddate'].'23:59:59').' ';
		$pagearr['enddate'] = $in['enddate'];
	}

	$sqlnum = "SELECT count(*) AS allrow FROM ".$sdbname.DATATABLE."_order_storage where CompanyID = ".$cid." ".$sqlmsg." ";
	$datasql   = "SELECT StorageID,StorageSN,StorageProduct,StorageAttn,StorageAbout,StorageUser,StorageDate FROM ".$sdbname.DATATABLE."_order_storage where CompanyID = ".$cid." ".$sqlmsg." Order by StorageID Desc";
...
?>
                <tr id="line_<? echo $lsv['StorageID'];?>" class="bottomline" onmouseover="inStyle(this)"  onmouseout="outStyle(this)" >
                  <td >&nbsp;<span class="selectinput"><input type="checkbox" name="selectedID[]" id="select_<? echo $lsv['StorageID'];?>" value="<? echo $lsv['StorageID'];?>" /></span>&nbsp;<? echo $n++;?></td>
				  <td height="28" >
					<span ><? echo $lsv['StorageID'];?></span>
				  </td>
				  <td><? echo $lsv['StorageSN'];?></td>
                  <td >		
						<span ><? echo $lsv['StorageUser'];?></span>
				  </td>

                  <td >
					<? echo date("Y-m-d H:i",$lsv['StorageDate']);?>		
				  </td>
				  <td>[<a href="javascript:void(0)" onclick="delete_data('<?php echo $lsv['StorageID'];?>');">删除</a>]</td>
                </tr>
	<? } }else{?>
     			 <tr>
	...
	<? }?>
              ...
              <table width="100%" border="0" cellspacing="0" cellpadding="0">
     			 <tr>
				   ...
				   <td  align="right"><? echo $page->ShowLink('show_library_data.php');?></td>
     			 </tr>
              </table>
