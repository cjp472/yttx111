...
<script type="text/javascript">
	...
	function delete_data(did)
	{
		if(confirm('确认删除吗?'))
		{
			...
			$.post("do_delete.php",
				{m:"delete_return", cid: cid, did:did},
				function(data){
					if(data == "ok"){
	...
	function delete_mul_data()
	{
		if(confirm('确认删除吗?'))
		{
			var cid = "<?php echo $cid;?>";
			$.blockUI({ message: "<p>正在执行，请稍后...</p>" }); 
			$.post("do_delete.php?m=delete_return", $("#MainForm").serialize(),
				function(data){
					if(data == "ok"){
						$.blockUI({ message: "<p>删除成功!</p>" });
...
<body>
			<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center" class="tc" >
        	  <form id="FormSearch" name="FormSearch" method="get" action="show_return_data.php">			  
				...
				<tr>
        	        ...
        	        <select name="ostatus" id="ostatus" class="selectline">
						...
						<?php
						foreach($return_status_arr as $key=>$var)
						{
					...
					<td width="220" nowrap="nowrap"><input type="text" name="begindate" id="begindate" class="inputline" style="width:100px;" value="<? if(!empty($in['begindate'])) echo $in['begindate'];?>" /> - <input type="text" name="enddate" id="enddate" class="inputline" style="width:100px;" value="<? if(!empty($in['enddate'])) echo $in['enddate'];?>" /></td>
...
<?php	
	...
	if(!empty($in['kw'])){	
		...
		if($in['stype']=='idsn'){
			...
			$sqlmsg .=  " AND ReturnID IN(".$in['kw'].")";
		}else{
			$in['kw'] = str_replace(",","','",$in['kw']);
			$sqlmsg .=  " AND ReturnSN IN('".$in['kw']."')";
		}
	}
	if(isset($in['ostatus']) and $in['ostatus'] != ''){
		$sqlmsg .= ' and ReturnStatus = '.$in['ostatus'].' ';
		$pagearr['ostatus'] = $in['ostatus'];
	}

	if(!empty($in['begindate']))
	{
		$sqlmsg .= ' and ReturnDate > '.strtotime($in['begindate'].'00:00:00').' ';
		$pagearr['begindate'] = $in['begindate'];
	}
	if(!empty($in['enddate']))
	{
		$sqlmsg .= ' and ReturnDate < '.strtotime($in['enddate'].'23:59:59').' ';
		$pagearr['enddate'] = $in['enddate'];
	}

	$sqlnum = "SELECT count(*) AS allrow FROM ".$sdbname.DATATABLE."_order_returninfo where ReturnCompany = ".$cid." ".$sqlmsg." ";
	$datasql   = "SELECT ReturnID,ReturnSN,ReturnOrder,ReturnClient,ReturnSendType,ReturnSendStatus,ReturnAbout,ReturnProductW,ReturnProductB,ReturnTotal,ReturnStatus,ReturnDate,ReturnType FROM ".$sdbname.DATATABLE."_order_returninfo where ReturnCompany = ".$cid." ".$sqlmsg." Order by ReturnID Desc";
	...
	if(!empty($list_data))
	{
		$clientdata = $db->get_results("select ClientID,ClientCompanyName,ClientCompanyPinyi from ".DATATABLE."_order_client where ClientCompany=".$cid." and ClientFlag=0 order by ClientCompanyPinyi asc");
...
?>
                <tr id="line_<? echo $lsv['ReturnID'];?>" class="bottomline" onmouseover="inStyle(this)"  onmouseout="outStyle(this)" >
                  <td >&nbsp;<span class="selectinput"><input type="checkbox" name="selectedID[]" id="select_<? echo $lsv['ReturnID'];?>" value="<? echo $lsv['ReturnID'];?>" /></span>&nbsp;<? echo $n++;?></td>
				  <td height="28" >
					<span ><? echo $lsv['ReturnID'];?></span>
				  </td>
				  <td><? echo $lsv['ReturnSN'];?></td>
                  <td >		
						<span><? echo $clientarr[$lsv['ReturnClient']];?></span>
				  </td>

                  <td >
				  <?php 
				  echo "<span title='金额' class=font12>¥ ".$lsv['ReturnTotal']."</span> ";
				  ?>
					</td>
                  <td >
					<? echo $return_status_arr[$lsv['ReturnStatus']];?>			
				  </td>
				  <td>[<a href="javascript:void(0)" onclick="delete_data('<?php echo $lsv['ReturnID'];?>');">删除</a>]</td>
                </tr>
	<? } }else{?>
     			 <tr>
	...
	<? }?>
              ...
              <table width="100%" border="0" cellspacing="0" cellpadding="0">
     			 <tr>
				   ...
				   <td  align="right"><? echo $page->ShowLink('show_return_data.php');?></td>
     			 </tr>
              </table>
