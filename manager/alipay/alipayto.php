...
$dbuser	 = dbuserconnect::dataconnect()->getdb();
$ip = $_SERVER["REMOTE_ADDR"];
$in = $inv->_htmlentities($in);

/*以下参数是需要通过下单时的订单数据传入进来获得*/
//必填参数
$out_trade_no = $in['paysn'];		//请与贵网站订单系统中的唯一订单号匹配
$subject      = $in['aliorder'];	//订单名称，显示在支付宝收银台里的“商品名称”里，显示在支付宝的交易管理的“商品名称”的列表里。
$body         = $in['alibody'];	//订单描述、订单详细、订单备注，显示在支付宝收银台里的“商品描述”里
$total_fee    = abs(floatval($in['alimoney']));	//订单总金额，显示在支付宝收银台里的“应付总额”里

if(empty($total_fee)) exit('支付金额错误!');

$isin = $dbuser->query("insert into ".DATATABLE."_order_pay(PayCompany,PayUser,PaySN,PayOrder,PayBody,PayMoney,PayType,PayMethod,PayDate) values( ".$_SESSION['uinfo']['ucompany'].",".$_SESSION['uinfo']['userid'].",'".$out_trade_no."','".$in['aliorder']."','".$in['alibody']."','".$in['alimoney']."','".$in['paytype']."','online',".time().")");
