...
if(!intval($in['ID']))
{
	$info = $db->get_row("SELECT * FROM ".$basedatabasename.DATATABLE."_order_article where ArticleCompany=1 and ArticleSort=9 and ArticleFlag=0 order by ArticleOrder desc, ArticleID asc limit 0,1");
}else{	 
	$info = $db->get_row("SELECT * FROM ".$basedatabasename.DATATABLE."_order_article where ArticleCompany=1 and ArticleSort=9 and ArticleFlag=0 and ArticleID=".intval($in['ID'])." limit 0,1");
}
?>
...
<ul>
	...
	<?
	$sinfo = $db->get_results("SELECT ArticleID,ArticleTitle FROM ".$basedatabasename.DATATABLE."_order_article where ArticleCompany=1 and ArticleSort=9 and ArticleFlag=0 order by ArticleOrder desc, ArticleID asc limit 0,50");
...
<!-- tree -->   
		...
		<fieldset title="" class="fieldsetstyle" style="padding:12px; width:96%;">
		<legend>操作指南</legend>
