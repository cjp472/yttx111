		...
		$.post("do_product.php",
			...
			data = Jtrim(data);
				if(data == "error"){
					...
					var delline = "line_" + pid;
					$("#"+delline).hide();
					$.blockUI({ message: "<p>还原成功!</p>" , css:{ 'left':'48%' }});
				}else{
					$.blockUI({ message: "<p>"+data+"</p>" , css:{ 'left':'48%' }});
				}					
			}		
 ...
 function going(o, sid)
 {
		...
		switch(o) {
			...
			case 'save':				
					...
					}else if(parseInt($('#data_Conversion').val()) > 0 && $('#data_Casing').val() == ''){
						$.blockUI({ message: "<p>请填写大单位名称</p>" });
					}else if( $('#data_Casing').val() != '' && ($('#data_Conversion').val() == '' || parseInt($('#data_Conversion').val()) <= 0) ){
						$.blockUI({ message: "<p>请填写换算单位,换算单位必须大于0</p>" });
...
{	
	...
	$.blockUI({ 
		message: $('#windowForm'),
		css:{ 
                width: '500px',height:'470px',top:'10%',left: _left + 'px'
            }			
		});
...
function set_level_price(fildname)
{
	...
	$.blockUI({ 
		message: $('#windowForm'),
		css:{ 
                width: '500px',height:'440px',top:'10%',left: _left + 'px'
            }			
		});
		$('#windowForm').css("width","500px");
	$('.blockOverlay').attr('title','点击返回').click($.unblockUI); 
...
function set_shield_client()
{
	...
	$.blockUI({ 
		message: $('#windowForm'),
		css:{ 
                width: '620px',height:'445px',top:'2%',left: _left + 'px'
            }			
		});
...
function do_change_price4(iid)
{
	...
	$('#windowContent').html('<iframe src="change_product_client_price.php?pid='+iid+'" width="600" marginwidth="0" height="530" marginheight="0" align="middle" frameborder="0" scrolling="no"></iframe>');
	$("#windowtitle").html('经销商单独指定价格');
	$.blockUI({ 
		message: $('#windowForm'),
		css:{ 
                width: '600px',height:'560px',top:'5%',left: _left + 'px'
            }			
		});
...
function do_save_brand()
{
	...
	if($('#data_BrandNO').val()=="")
	{
		$.blockUI({ message: "<p>请先输入编号！</p>", css:{ 'left':'48%' }});
	}else if(!/^[A-Za-z0-9]*$/.test($('#data_BrandNO').val())){
        $.blockUI({ message: "<p>只能输入数字或字母！</p>", css:{ 'left':'48%' }});
    }else if($('#data_BrandName').val()==""){
		$.blockUI({ message: "<p>请先输入品牌名称！</p>", css:{ 'left':'48%' }});
	}else{
		$.blockUI({ message: "<p>正在执行，请稍后...</p>", css:{ 'left':'48%' }});
...
//
//	}else
	if(!/^[A-Za-z0-9]*$/.test($('#edit_BrandNO').val())){
		...
		$.post("do_product.php",
			...
			function(data){
				...
				if(data == "ok"){
					...
					$.blockUI({ message: "<p>"+data+"</p>" , css:{ 'left':'47%' }});
				}					
			}		
		);
	}else{
