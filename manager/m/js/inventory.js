...
function select_product()
{
	...
	$.blockUI({ 
		message: $('#windowForm6'),
		css:{ 
                width: '620px',height:'480px',top:'5%',left: _left + 'px'
            }			
		});
	$('#inputsp').val('');
...
function change_lib_num(idd,oldnumber)
{
	...
	if(newnumber != oldnumber || (newnumber != oldnumber_h && oldnumber_h != 0))
	{
		$.growlUI('正在执行，请稍候...');
		$.post("do_inventory.php",
...
function out_warning_excel()
{
	document.MainForm.action = 'warning_excel.php';
    document.MainForm.submit();
}

function out_warning_ms_excel()
{
	document.MainForm.action = 'warning_ms_excel.php';
    document.MainForm.submit();
}