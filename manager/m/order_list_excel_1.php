...
$objActSheet->getColumnDimension('S' )->setWidth(50);
...
foreach($clientdata as $cvar)
{
	...
	$clientNo[$cvar['ClientID']] = $cvar['ClientNO'];
		...
		$odata = $db->get_results("SELECT * FROM ".DATATABLE."_order_orderinfo where OrderCompany = ".$_SESSION['uinfo']['ucompany']." and OrderID in (".$idmsg.") limit 0,50");
		...
		$objActSheet->setCellValue('S'.$k, '留言');

		$objActSheet->getStyle('A'.$k.':S'.$k)->getFont()->setBold(true);
		$objActSheet->getStyle('A'.$k.':S'.$k)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB('FFEEEEEE');
		$objActSheet->freezePane('A3');
	...
	{	
		foreach($odata as $var)
		{
		    ...
		    //增加留言信息输出 addby lxc 20160428 START
		    $strMessage = '';
		    $submitdata = $db->get_results("select ID,AdminUser,Name,Date,Status,Content from ".DATATABLE."_order_ordersubmit where Status LIKE '%留言' AND CompanyID=".$_SESSION['uinfo']['ucompany']." and OrderID=".$var['OrderID']." order by ID ASC");
		    if($submitdata)
		    {
		    	foreach ($submitdata as $v)
		    		$strMessage .= $v['Name'].'—'.$v['Content'].'；';
		    }
		    $objActSheet->setCellValue('S'.$k, $strMessage);
		    $objActSheet->getStyle('S'.$k)->getAlignment()->setWrapText(true);
		    /****** END *********/
		    
		}
		$objActSheet->getStyle('A2:S'.$k)->getFont()->setSize(10);
	}
		...
		$objPHPExcel->getActiveSheet()->getStyle('A2:S'.$k)->applyFromArray($styleThinBlackBorderOutline);
