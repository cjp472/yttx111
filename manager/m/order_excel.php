...
// 			$objActSheet->setCellValue('D'.$k, '规格');
			$objActSheet->setCellValue('C'.$k, '品牌');
			$objActSheet->setCellValue('D'.$k, '型号');
			...
			foreach($cartdata as $ckey=>$cvar)
			{
				...
				$objActSheet->setCellValueExplicit('C'.$baseRow , $brandsqlarr[$cvar['BrandID']]['BrandName'],PHPExcel_Cell_DataType::TYPE_STRING);
				$objActSheet->setCellValue('D'.$baseRow, $cvar['Model']);
		...
		$objActSheet->setCellValue('B'.$baseRow, '商品名称');
// 		$objActSheet->setCellValue('C'.$baseRow, '生产厂家');
// 		$objActSheet->setCellValue('D'.$baseRow, '规格');
		$objActSheet->setCellValue('C'.$baseRow, '品牌');
		$objActSheet->setCellValue('D'.$baseRow, '型号');
		$objActSheet->setCellValue('E'.$baseRow, '单位');
		$objActSheet->setCellValue('F'.$baseRow, '数量');
		$objActSheet->setCellValue('G'.$baseRow, '包装');
		$objActSheet->setCellValue('H'.$baseRow, '单价');
		$objActSheet->setCellValue('I'.$baseRow, '折扣');
		$objActSheet->setCellValue('J'.$baseRow, '折后价');
		$objActSheet->setCellValue('K'.$baseRow, '小计(元)');

		$objActSheet->getStyle('A'.$baseRow.':K'.$baseRow)->getFont()->setBold(true);
		$objActSheet->getStyle('F'.$baseRow.':K'.$baseRow)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
