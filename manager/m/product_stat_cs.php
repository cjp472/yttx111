...
<body>      
    <div id="bodycontent">
		...
		<legend> 商品订购统计：</legend>
             <table width="100%" border="0" cellspacing="0" cellpadding="0">
				 ...
				 <?php
					...
					$statsql  = "SELECT sum(c.ContentNumber) as cnum,c.OrderID,c.ContentID,c.ContentName,c.ContentColor,ContentSpecification
                                ...
                                group by c.ContentColor,c.ContentSpecification ";
					$statdata = $db->get_results($statsql);

					$statsqlg  = "SELECT sum(c.ContentNumber) as cnum,c.OrderID,c.ContentID,c.ContentName,c.ContentColor,ContentSpecification
					...
					$statsqlr  = "SELECT sum(c.ContentNumber) as rnum,c.ContentID,c.ContentName,c.ContentColor,ContentSpecification
					...
					if(!empty($rdata))
					{
						...
						{
							$rarr[$rvar['ContentColor'].'_'.$rvar['ContentSpecification']] = $rvar['rnum'];
						}
					}
					...
					if(!empty($statdatag))
					{
						...
						{
							$garr[$rvar['ContentColor'].'_'.$rvar['ContentSpecification']] = $rvar['cnum'];
						}
					}
					...
					if(!empty($statdata))
					{
						...
						{
							if(empty($rarr[$var['ContentColor'].'_'.$var['ContentSpecification']])) $rarr[$var['ContentColor'].'_'.$var['ContentSpecification']] = 0;
							if(empty($garr[$var['ContentColor'].'_'.$var['ContentSpecification']])) $garr[$var['ContentColor'].'_'.$var['ContentSpecification']] = 0;
							if(empty($cmsg))
							{
								$cmsg = "'".$var['ContentColor'].'_'.$var['ContentSpecification']."'";
								$pmsg = ($var['cnum'] + $garr[$var['ContentColor'].'_'.$var['ContentSpecification']]);
								$rmsg  = $rarr[$var['ContentColor'].'_'.$var['ContentSpecification']];
							}else{
								$cmsg .= ",'".$var['ContentColor'].'_'.$var['ContentSpecification']."'";
								$pmsg .= ",".($var['cnum'] + $garr[$var['ContentColor'].'_'.$var['ContentSpecification']]);
								$rmsg  .= ",".$rarr[$var['ContentColor'].'_'.$var['ContentSpecification']];
		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
				chart = new Highcharts.Chart({
					...
					title: {
						text: '（<? echo $productinfo['Name'];?>） 订购数据'
					},
					xAxis: {
     			 ...
     			 <tr>
       				 <td height="30" bgcolor="#efefef" >&nbsp;&nbsp;&nbsp;&nbsp; <strong>（<? echo $productinfo['Name'];?>） 订购数据 </strong></td>
     			 </tr>
     			 <tr>
       				 <td >
               ...
               <thead>
                <tr>
                 ...
                 &nbsp;
				  ...
				  <td width="14%" class="bottomlinebold">颜色</td>
		...
		foreach($statdata as $var)
		{
			...
			$var['rnum'] = $rarr[$var['ContentColor'].'_'.$var['ContentSpecification']];
			if(empty($var['rnum'])) $var['rnum'] = 0;
			$var['gnum'] = $garr[$var['ContentColor'].'_'.$var['ContentSpecification']];
			if(empty($var['rnum'])) $var['rnum'] = 0;
			...
			$totalg = $totalg + $var['gnum'];
			 ...
			 <tr class="bottomline" onmouseover="inStyle(this)"  onmouseout="outStyle(this)">
				  ...
				  <td >&nbsp;<? echo $var['ContentColor'];?></td>
				  <td >&nbsp;<? echo $var['ContentSpecification'];?></td>
                  <td >&nbsp;<? echo $var['onum'];?></td>
				  <td >&nbsp;<? echo $var['gnum'];?></td>
				  <td >&nbsp;<? echo $var['rnum'];?></td>
				  <td >&nbsp;<? echo $var['cnum'];?></td>
			 ...
			 <tr class="bottomline" onmouseover="inStyle(this)"  onmouseout="outStyle(this)">
				 ...
				 <? }else{?>
     			 <tr>
       				 <td height="130" bgcolor="#ffffff" align="center" colspan="9">&nbsp; 暂无符合条件的数据!</td>
     			 </tr>
				<? }?>
              </table>
		</fieldset>
