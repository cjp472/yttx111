...
if(!intval($in['ID']))
{
	...
	$uinfo = $db->get_row("SELECT * FROM ".DATABASEU.DATATABLE."_order_user_made where UserCompany=".$_SESSION['uinfo']['ucompany']." and UserID=".intval($in['ID'])." limit 0,1");
}
...
function do_save_edit_user()
{
	...
	if($('#data_UserName').val()=="" )
	{
		...
		$.post("do_system_made.php?m=content_edit_save", $("#MainForm").serialize(),
			function(data){
				data = Jtrim(data);
				if(data == "ok"){
	      ...
	      <input name="UserID" type="hidden" id="UserID" value="<? echo $uinfo['UserID'];?>"  />
		   ...
		   <div id="searchline">
        	<div class="leftdiv width300">
        	 <div class="locationl"><strong>&nbsp;&nbsp;当前位置：</strong><a href="user_made.php">帐号管理</a> &#8250;&#8250; <a href="#">修改帐号</a></div>
   	        </div>
                ...
                <tr>
                  ...
                  <td bgcolor="#FFFFFF">
				   <div style="" id="show_sort_sel">
					...
					$csarr = null;
					if(!empty($uinfo['UserSite']))
					{
						$csarr = explode(",",$uinfo['UserSite']);
					}
					...
					if(!empty($topsort))
					{
						foreach($topsort as $v)
						{
								...
								{
									echo '<div style="width:25%;display: inline-block;" title="'.$v['SiteName'].'"><input style="width:15%" id="UserSite_'.$v['SiteID'].'" name="data_UserSite[]" type="checkbox" value="'.$v['SiteID'].'" checked="checked" />'.$v['SiteName'].'</div>';
								}else{
