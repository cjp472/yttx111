...
if($in['m']=="login" && !empty($in['su'])){
        $us = strtolower(trim($in['us']));
		$ip = RealIp();
		
	    $suinfo_sql = "select suning_user_name,spass from ".DB_DATABASEU.DATATABLE."_order_suninginfo where suning_user_name='".$in['su']."'";
		$result = $db->get_row($suinfo_sql);
		$ps=$result['spss'];
