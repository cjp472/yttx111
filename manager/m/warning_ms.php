...
<?php include_once ("top.php");?>
    <div id="bodycontent">
    	...
    	<div id="searchline">
        	<div class="leftdiv">
        	  <form id="FormSearch" name="FormSearch" method="post" action="warning_ms.php">
        	    <label>
			...
			<div class="location"><strong>当前位置：</strong><a href="warning.php">库存预警</a> &#8250;&#8250; <? if(empty($sortinfo)) echo "所有商品"; else echo $sortinfo['SiteName'];?></div>
        </div>
        ...
        <div id="sortleft">
<!-- tree --> 
<div class="linebutton"><input type="button" name="newbutton" id="newbutton" value="库存预警" class="bluebtn" onclick="javascript:window.location.href='warning.php'" /> </div> 
<hr style="clear:both;" />
<div id="sidetree"> 
<div class="treeheader">&nbsp;<strong><a href="warning_ms.php">所有商品</a></strong></div>  	  
        	  ...
        	  <table width="100%" border="0" cellspacing="0" cellpadding="0">               
               <thead>
                <tr>
				  ...
				  <td width="12%" class="bottomlinebold" >颜色</td>
				  <td width="12%" class="bottomlinebold" >规格</td>
...
<?php
	...
	$InfoDataNum = $db->get_row("SELECT count(*) AS allrow FROM ".DATATABLE."_order_content_index i INNER JOIN ".DATATABLE."_order_site s ON i.SiteID=s.SiteID inner join ".DATATABLE."_order_inventory_number n ON n.ContentID=i.ID where i.CompanyID = ".$_SESSION['uinfo']['ucompany']."  and i.FlagID=0 ".$sqlmsg." and n.ContentNumber < i.LibraryDown ");

	$datasql   = "SELECT i.ID,i.Name,i.Coding,i.Units,i.Color,i.Specification,".$fieldsql." n.ContentColor,n.ContentSpec,n.OrderNumber,n.ContentNumber FROM ".DATATABLE."_order_content_index i INNER JOIN ".DATATABLE."_order_site s ON i.SiteID=s.SiteID inner join ".DATATABLE."_order_inventory_number n ON n.ContentID=i.ID where i.CompanyID = ".$_SESSION['uinfo']['ucompany']." and i.FlagID=0 ".$sqlmsg." ORDER BY i.OrderID DESC, i.ID DESC";
...
?>
             <tr  class="bottomline" onmouseover="inStyle(this)"  onmouseout="outStyle(this)">
				...
				<td ><?  echo $basecode1 = base64_decode(str_replace($rp,$fp,$lsv['ContentColor'])); ?>	</td>				
				<td ><?  echo $basecode2 = base64_decode(str_replace($rp,$fp,$lsv['ContentSpec'])); ?>	</td>
...
<?php }?>
              ...
              <table width="100%" border="0" cellspacing="0" cellpadding="0">
     			 <tr>
   			       ...
   			       <td width="8%" >全选/取消</td>
   			       <td width="20%" class="sublink"><ul><li><a href="javascript:void(0);" onclick="out_warning_ms_excel();" >批量导出</a></li></ul></td>
				   <td align="right"><? echo $page->ShowLink('warning_ms.php');?></td>
     			 </tr>
              </table>
...
function ShowTreeMenu($resultdata,$p_id) 
{
	...
	foreach($resultdata as $key => $var)
	{
		if($var['ParentID'] == $p_id)
		{
			if($var['ParentID']=="0")
			{
				$frontMsg  .= '<li><a href="warning_ms.php?sid='.$var['SiteID'].'"><strong>'.$var['SiteName'].'</strong></a>';
			}else{
				$frontMsg  .= '<li><a href="warning_ms.php?sid='.$var['SiteID'].'">'.$var['SiteName'].'</a>';
			}
			$frontMsg2  = "";
