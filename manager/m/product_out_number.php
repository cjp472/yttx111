...
$InfoDataNum = $db->get_row("SELECT count(*) AS allrow FROM ".DATATABLE."_order_content_index where CompanyID = ".$_SESSION['uinfo']['ucompany']." and FlagID=0 ");
...
<div style="width:100%; height:340px; overflow:auto;">
          <form id="MainForm" name="MainForm" method="post" action="" target="" >
        	  <table width="100%" border="0" cellpadding="2" cellspacing="1" bgcolor="#CCCCCC" class="tc">               
                <thead>
				<tr>
				  <td width="30%" >导出商品数据：</td><td width="40%"><? echo '共 '.$InfoDataNum['allrow'].' 条数据';?></td><td width="30%">&nbsp;操作</td>				  
                </tr>
				</thead>
				 <?php 
				 ...
				 ?>
				   <tr>
					...
					<td >【<a href="product_excel.php?ty=all&b=<? echo $i;?>" target="_blank" > 导出数据</a>】 </td>
				   </tr>
				<? }?>			
              </table>			
              </form>
