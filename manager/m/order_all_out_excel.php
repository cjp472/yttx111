...
if($in['ty'] == "all")
{
	if(empty($in['b']))
	{
		...
		$odata = $db->get_results("SELECT c.ClientID,c.ClientCompanyName,o.* FROM ".DATATABLE."_order_orderinfo AS o LEFT JOIN ".DATATABLE."_order_client as c ON o.OrderCompany = c.ClientCompany AND o.OrderUserID = c.ClientID  where o.OrderCompany = ".$_SESSION['uinfo']['ucompany']." order by OrderID desc".$endsql);
		$k = 1;
		$k++;
	...
	{	
		foreach($odata as $var)
		{
			...
			$objActSheet->setCellValue('B'.$k, $var['ClientCompanyName']);
