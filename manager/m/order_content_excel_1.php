...
if(empty($in['ty'])){
	$sqlc = "select * from ".DATATABLE."_view_index_cart where CompanyID=".$_SESSION['uinfo']['ucompany']." and OrderID=".$oinfo['OrderID']." order by SiteID asc,ID asc";
}else{
	$sqlc = "select * from ".DATATABLE."_view_index_cart where CompanyID=".$_SESSION['uinfo']['ucompany']." and OrderID=".$oinfo['OrderID']." order by ID asc";
}

$cartdata = $db->get_results($sqlc);
...
$objActSheet->getStyle('F2:'.$endc.'3')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
	...
	foreach($cartdata as $ckey=>$cvar)
	{
		...
		if($_SESSION['uinfo']['ucompany']=="115") $cvar['Barcode'] = totiaoma($cvar['Coding'],$cvar['ContentColor'],$cvar['ContentSpecification']);
		//
		if($_SESSION['uinfo']['ucompany']=="115"){
			if(!empty($cvar['ContentColor']))  $cvar['ContentColor'] = $cvar['ContentColor'].' '.toma($cvar['ContentColor']);
			if(!empty($cvar['ContentSpecification']))  $cvar['ContentSpecification'] = $cvar['ContentSpecification'].' '.tomasp($cvar['ContentSpecification']);
		}
...
function toma($pcolor)
{
	$color_arr = array(
	 ...
	 $rmsg = '';
	 $pcolor = str_replace('其他','其它',$pcolor);
	 if(!empty($pcolor)) $rmsg =  array_search($pcolor,$color_arr);
	 //if(!empty($pspec)) $rmsg =  $rmsg.$pspec;
	 return $rmsg;
}

function tomasp($pspec)
{
	$spec_arr = array(
		'00'			=>  '均码',
		'01'			=>  'S',
	 ...
	 //$color_data_arr = array_flip($color_arr);
	 $rmsg = '';
	 if(!empty($pspec)) $rmsg =  array_search($pspec,$spec_arr);
	 if(empty($rmsg)) $rmsg =  $pspec;
	 return $rmsg;
}
...
function totiaoma($pno,$pcolor,$pspec)
{
	 $rmsg = $pno;
	 $pcolor = str_replace('其他','其它',$pcolor);
	 if(!empty($pcolor)) $rmsg =  $rmsg.toma($pcolor);
	 if(!empty($pspec))  $rmsg  =  $rmsg.tomasp($pspec);
	 return $rmsg;
}
?>