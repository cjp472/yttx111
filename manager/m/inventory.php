...
if(!empty($in['numw'])){
	...
	//editby lxc 2016-04-18
	$strMsg = '';
	if($in['num'] == 0 && $in['numw'] == 'EQ')
		$strMsg = ' IS NULL ';

	if(substr($in['numw'],0,1)=='K'){
		$sqlmsg .= " and (n.OrderNumber ".$darr[$in['numw']]." ".$in['num'].(empty($strMsg) ? '' : ' OR n.OrderNumber'.$strMsg)." )";
	}else{
		$sqlmsg .= " and (n.ContentNumber ".$darr[$in['numw']]." ".$in['num'].(empty($strMsg) ? '' : ' OR n.OrderNumber'.$strMsg)." )";
	}	
...
<script type="text/javascript">
	...
	$(function() {
        ...
        $("#searchbutton").click(function(){
            document.FormSearch.action="inventory.php";
            document.FormSearch.method = "get";
            document.FormSearch.submit();
        });

        $("#export_excel").click(function(){
            document.FormSearch.action="do_inventory.php?m=export_excel";
			...
			<table width="98%" border="0" cellspacing="0" cellpadding="0" align="center" >
        	  <form id="FormSearch" name="FormSearch" method="get" action="inventory.php">
        		...
        		<tr>
					...
					<td width="70"><input name="searchbutton" type="submit" class="mainbtn" id="searchbutton" value="搜 索" /></td>
                    <td width="70"><input class="mainbtn" type="button" id="export_excel" value="导 出"/></td>
				</tr>
   	          </form>
			 </table>
        </div>
        	  ...
        	  <table width="100%" border="0" cellspacing="0" cellpadding="0">               
               <thead>
                <tr>
				  ...
				  <td width="7%" class="bottomlinebold" align="right">库存上限</td>
				  <td width="7%" class="bottomlinebold" align="right">库存下限</td>
...
<?php
    ...
    $page->LinkAry = array("kw"=>$in['kw'],"sid"=>$in['sid'],"sc"=>$in['sc'],"numw"=>$in['numw'],"num"=>$in['num'],"BrandID"=>$in['BrandID'],"sp"=>$in['sp']);        
	$datasql   = "SELECT i.ID,i.SiteID,i.Name,i.Coding,i.Price1,i.Price2,i.Units,i.Casing,i.Color,i.Model,i.Specification,i.LibraryDown,i.LibraryUp,n.OrderNumber,n.ContentNumber FROM ".DATATABLE."_order_content_index i left join ".DATATABLE."_order_site s ON i.SiteID=s.SiteID left join ".DATATABLE."_order_number n on i.ID=n.ContentID  where i.CompanyID = ".$_SESSION['uinfo']['ucompany']."  and i.FlagID=0 ".$sqlmsg." ".$sarr[$in['sc']];
...
?>
                <tr id="line_<? echo $lsv['ID'];?>" class="bottomline" onmouseover="inStyle(this)"  onmouseout="outStyle(this)">
                  ...
                  <td align="right" class="bold" title="无子库存" <?php if($lsv['LibraryDown'] > $lsv['ContentNumber']) echo 'style="color:red;"';?> ><? if(empty($lsv['ContentNumber'])) echo "0"; else echo $lsv['ContentNumber'];?>&nbsp;</td>
				  <? 
					}else{
				  ...
				  <? }}?>
				  <td align="right"><? echo $lsv['LibraryUp'];?>&nbsp;</td>
				  <td align="right"><? echo $lsv['LibraryDown'];?>&nbsp;</td>
				...
				$page_total   = $page_total + $linetotal;
				 ...
				 ?>
     			 <tr class="bottomline">
					 <td ></td><td ></td>
       				 <td colspan="2" height="30" class="font12h">本页小计：</td>
