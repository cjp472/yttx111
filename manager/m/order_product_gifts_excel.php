...
if(empty($in['selectedID']))
{
	...
	$datasql   = "SELECT Name,Coding,Casing,Barcode,Units,OrderID,ClientID,ContentColor,ContentSpecification,ContentPrice,ContentNumber,ContentSend FROM ".DATATABLE."_view_index_gifts where CompanyID = ".$_SESSION['uinfo']['ucompany']."  and ID IN (".$idmsg.") order by ID desc limit 0,50";
	...
	$osql = "select OrderID,OrderSN from ".DATATABLE."_order_orderinfo where OrderCompany = ".$_SESSION['uinfo']['ucompany']." and OrderID IN (".$oidarr.")";
	$odata = $db->get_results($osql);
	foreach($odata as $v){
		$odataarr[$v['OrderID']] = $v['OrderSN'];
	}
}
...
if(!empty($list_data))
{
	...
	foreach($list_data as $var)
	{
		...
		$objActSheet->setCellValue('J'.$k, $odataarr[$var['OrderID']]);
	}
...
$filename = '订单赠品明细_'.date("Ymd").'_'.rand(1,999).'.xls';
