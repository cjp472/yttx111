...
if($in['ty'] == "all")
{
	...
	$datasql   = "SELECT * FROM ".DATATABLE."_view_index_content where CompanyID = ".$_SESSION['uinfo']['ucompany']." and FlagID=1 order by OrderID desc,ID asc ".$endsql;
	...
	if(empty($in['selectedID']))
	{
		...
		$datasql   = "SELECT * FROM ".DATATABLE."_view_index_content where CompanyID = ".$_SESSION['uinfo']['ucompany']." and FlagID=1 and ID IN (".$idmsg.") order by OrderID desc,ID asc";
	}
}
