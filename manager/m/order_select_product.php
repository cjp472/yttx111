...
if(empty($in['oid']))
{
	...
	if($oinfo['OrderStatus'] > 1 && $oinfo['OrderStatus']!=9) exit('错误地址!');
...
body { margin:0; padding:0; font-size:12px; background-color:#ffffff; font-family: "微软雅黑", Arial, Helvetica, sans-serif, "宋体"; color:#333;background:#fff;}
        	  ...
        	  <table width="98%" border="0" cellpadding="2" cellspacing="1" bgcolor="#CCCCCC">               
               <thead>
                <tr>
                  ...
                  <!-- <td width="24%" bgcolor="#efefef" >生产厂家</td>	 -->			  
                  <!-- <td width="14%" bgcolor="#efefef" >规格</td> -->				  
                  <td width="24%" bgcolor="#efefef" >品牌</td>				  
                  <td width="14%" bgcolor="#efefef" >型号</td>				  
...
<?
	...
	$datasql   = "SELECT ID,SiteID,BrandID,Name,Coding,Price1,Price2,Price3,Units,Casing,Model,Color,Specification FROM ".DATATABLE."_order_content_index where CompanyID = ".$_SESSION['uinfo']['ucompany']." ".$sqlmsg." and FlagID=0 ORDER BY OrderID DESC, ID DESC";
...
?>
                <tr id="line_<? echo $lsv['ID'];?>"  >
                  ...
                  <td bgcolor="#FFFFFF" ><? echo $brandsqlarr[$lsv['BrandID']]['BrandName'];?>&nbsp;</td>
                  <td bgcolor="#FFFFFF" ><? echo $lsv['Model'];?>&nbsp;</td>
				  ...
				  <td align="center" bgcolor="#FFFFFF">
                     <!-- <a onclick="javascript:addtocart_select('<?/* echo $lsv['ID'];*/?>','<?/* echo $oinfo ['OrderID'];*/?>','<?/* echo $cinfo['ClientID'];*/?>');" href="javascript:void(0);"><strong>&#8250; 订购</strong>
                      </a>-->&nbsp;
                      <a id="shareit_<?php echo $lsv['ID']; ?>" href="javascript:;" onclick="shortToCart('<?php echo $lsv['ID']; ?>','<?php echo (empty($lsv['Color']) && empty($lsv['Specification'])) ? "N" : "Y"; ?>','<?php echo $in['oid']; ?>');"><strong>&#8250; 订购</strong></a>
                  </td>
              </tr>
			<? } }else{?>
						  <tr>
    ...
    <script type="text/javascript">
        function shortToCart(pid,cstype,oid)
        {
            if(cstype=="N")
            {
                showinnumcart(pid);
                ...
                function showCartDiv(pid,oid)
                {
                    ...
                    $.blockUI({
                        message: $('#windowForm'),
                        css:{
                            width: '540px',height:'350px',top:'20px',left:'110px'
                        }
                    });
