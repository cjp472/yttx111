...
class YOpenApi{
	...
	protected 	$predefine			= array(								//每个模块对应的服务代码
										"setAccount"	=> array("service" => "qftDealerRegister"),//"setAccount"	=> array("service" => "cowpayUserRegister"), tubo 修改开户为新接口 2015-10-15
...
// 										"payOrder"		=> array("service" => "qftIntegratedPayment"),//"payOrder"		=> array("service" => "commonTradePay"),
										"payOrder"		=> array("service" => "commandPayTradeCreatePay"),//"payOrder"		=> array("service" => "commonTradePay"),
										"createOrder"	=> array("service" => "createTradeOrder"),
										"setBind"		=> array("service" => "signmanybank"),
										"managerBind"	=> array("service" => "signPactManager"),
										"wallet"		=> array("service" => "wallet"),//tubo 增加跳转登录
										"paySDK"		=> array("service" => "qftSDKPayment"),//tubo 增加跳转登录
	                                    "certifyStatus"	=> array("service" => "userInfoQuery"),//wanjun 查询用户信息
	                                    "openPaymentAccount"	=> array("service" => "openPaymentAccount"),//wanjun 供应商在线开户
	                                    "ppmNewRuleRegisterUser"	=> array("service" => "ppmNewRuleRegisterUser"),//wanjun 经销商在线开户
	 ...
	 * @todo 需要增加参数错误时的异常处理
	 */
	public function __construct($companyID = 0, $backgroundType = 'front', $isSDK = ''){
	 ...
	 */
	protected function createID($isSDK = ''){
		
		if(!empty($isSDK)){
			list($usec, $sec) = explode(" ", microtime());
			$this->orderNo = date('YmdHis').substr($usec, 4);
		}else{
			list($usec, $sec) = explode(" ", microtime());
			$this->orderNo = YAPI_ORDER_PREFIX.date('YmdHis').substr($usec, 4);
		}
	...
	protected function myPrefix(){
		...
		if($this->backgroundType == 'front'){//经销商端
			...
			$prefix = $env == 'm' ? YAPI_ORDER_PREFIX_BACKEND : YAPI_ORDER_PREFIX_BACKEND;
		}
