...
$log->logInfo('allinpay-in', $in);
	...
	$in['orderMoney'] = $pinfo['OrderMoney'];
	...
	$log->logInfo('allinpay-result', $verify_Result.','.$pay_Result);
	//保存网银支付信息
	function save_netpay($inv,$pinfo,$db,$log)
	{
		if($pinfo['PayResult'] != '1')
		{
			...
			$log->logInfo('allinpay-sqlin', $sqlin);
	...
	include template("pickup_netpay");
