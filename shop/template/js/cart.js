...
function changenum(gid,type)
{
	if(gid !='')
	{
		var num = $('#shareit-field-'+gid).val();
		if(num != '' && parseInt(num) != 'NaN'){
			if(type == 'add')
				$('#shareit-field-'+gid).val(parseInt(num)+1);
			else if(type == 'sub' && parseInt(num) > 1)
				$('#shareit-field-'+gid).val(parseInt(num)-1);
			else
				$('#shareit-field-'+gid).val(1);
		}else
			$('#shareit-field-'+gid).val(1);
	}
}
...
function saveonetocart()
{
		...
		$.post("cart.php",
			...
			function(data){
				if(data.backtype=='ok'){
					$.growlUI('该商品预订成功！您目前共订购 <span style="color: #ff9844;">'+data.cartnum+' </span>种商品。<br />点击 <a href="cart.php" target="_parent" style="color: #ff9844;text-decoration:none;">查看购物车</a> 浏览已预订的商品。', '', 10000000);
					carths(data.cartnum);						
				}else{
...
function updatecartnumber()
{
		$.post("cart.php",
			...
			function(data){
				data = Trim(data);
				if(data.length < 5){
					$.growlUI('该商品预订成功！您目前共订购  <span style="color: #ff9844;">'+data+' </span>种商品。<br />点击 <a href="cart.php" target="_parent" style="color: #ff9844;">查看购物车</a> 浏览已预订的商品。');
					carths(data);	
					closewindowui();
				}else{
					$.growlUI(data);
...
function updatecartsub()
{
		...
		var OrderAmount=$("#all_price_total").attr("data-total");
		$('#price_all_').val(OrderAmount);
...
function addtowishlist(pid)
{
		$.post("wishlist.php",
			...
			function(data){
				data = Trim(data); 
				if(data == "ok"){
					$.growlUI('收藏成功！<br />点此可以 <a href="wishlist.php" style="color:#ff9844;">查看</a> 我收蒧的商品...');
					if($('#to-fav').length) $('#to-fav').css('color', 'red'); 
				}else{
					$.growlUI(data);
				}
			}			
...
function removewishlist(pid, islist)
{	
...
function shownoticediv(gid)
{
		...
		$.blockUI({ 
			message: $('#windowForm'),
			css:{ 
					width: '340px',height:'220px',top:'15%', left: left+'px'
				}			
			});
...
function savegoodtocart(pid)
{
	var pnum = $('#shareit-field-'+pid).val();
	var pcolor = $('#color-'+pid).val();
	var pspec = $('#spec-'+pid).val();
	
	if(parseInt(pid) != 'NaN' && parseInt(pnum) != 'NaN' && parseInt(pnum) > 0){
		$.growlUI('正在操作，请稍候...');
		$.post("cart.php",
			{m:"addtocart", pid: pid, pnum:pnum, pcolor: pcolor, pspec: pspec },
			function(data){
				if(data.backtype=='ok'){
					$.growlUI('该商品预订成功！您目前共订购 <span style="color: #ff9844;">'+data.cartnum+'</span> 种商品。<br />点击 <a href="cart.php" target="_parent" style="color: #ff9844">查看购物车</a> 浏览已预订的商品。');
					carths(data.cartnum);						
				}else{
					$.growlUI(data.cartnum);
				}
			...
			$.post("cart.php?m=updatecart&page="+page, $("#formcart").serialize(),
				function(data){
					...
					//if(data.length < 5){
						...
						$("#all_price_total").attr("data-total",ptotal);
...
function select_product()
{
	...
	$('#windowContent').html('<iframe src="list.php?m=select&kw='+$('#inputsp').val()+'&selectid='+$('#selectid_storage').val()+'" width="100%" marginwidth="0" height="460" marginheight="0" align="middle" frameborder="0" scrolling="no"></iframe>');
	$.blockUI({ 
		message: $('#windowForm6'),
		css:{ 
                width: '850px',height:'490px',top:'8%',left: left+'px'
            }			
		});
	$('#inputsp').val('');
	$('.blockOverlay').attr('title','点击返回').click($.unblockUI); 
