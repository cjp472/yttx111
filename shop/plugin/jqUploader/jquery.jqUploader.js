 ...
 jQuery.fn.jqUploader = function(options) {
    return this.each(function(index) {
		...
		var opts = jQuery.extend({
				...
				allowedExt:	      '*.gif; *.jpg; *.jpeg; *.png', // allowed extensions
				allowedExtDescr:  'Images (*.gif; *.jpg; *.jpeg; *.png)',
				params:           {menu:false},
				flashvars:        {},
				hideSubmit:       true,
				barColor:		      '0000CC',
				...
				errorSizeMessage: '文件太大，请改小再上传!',
				validFileMessage: '点击 upload 按钮上传!',
		...
		TerminateJQUploader = function(containerId,filename,varname){
			...
			$this.text('').append('<span style="color:#00CC00">已上传 <strong>'+filename+'</strong> ! (the filename is now stored in the form as an hidden input field)</span><input name="'+varname+'" type="hidden" id="'+varname+'" value="'+filename+'"/>');
