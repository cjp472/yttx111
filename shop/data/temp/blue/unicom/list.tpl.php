...
<? $prepage  = $goodslist['curpage'] > 1 ? $goodslist['curpage']-1 : 1 ?>
        
<? $nextpage = $goodslist['curpage'] < $goodslist['totalpage'] ? $goodslist['curpage']+1 : $goodslist['totalpage'] ?>
...
<input class="f-r xiala m-r m-t-5"  size="80" type="radio" name="stock" 
<? if(empty($in['stock'])) { ?>
  value='0' checked='checked' placeholder="First name" style="width: 20px;height: 20px;border: 1px solid #f9f9f9;margin-top: 13px;background: url(template/tpl/unicom/hcheck.png) no-repeat;cursor:pointer;" 
<? } else { ?>
 value='1' style="width: 20px;height: 20px;border: 1px solid #f9f9f9;margin-top: 13px;float:right;background: url(template/tpl/unicom/oracheck.png) no-repeat;cursor:pointer;"
<? } ?>
 onclick="javascript:(this.value==1) ? this.value=0 : this.value=1 ;submit();">
<? if($in['t'] == "textlist") { ?>
