...
当前余额：<b class="a-btn" style="font-size:18px;"><? echo commondata::get_amount(); ?></b>&nbsp;元 <a class="a-btn" href="finance.php?m=new&t=y" > [充值] </a>
...
<form name="searchform" id="searchform" action="brand.php" method="get">
...
html+='<li class="select_li"><p style="float: left;">'+item.BrandName+'</p><p>约有<span>'+item.c_num+'</span>个结果</p></li>';
...
window.location.href="brand.php?kw="+select_vals;
});
...
<a href="list.php"> 品牌街 </a>
<? if(is_array($location)) { foreach($location as $lkey => $lvar) { ?>
...
<div class="conmed">
<ul>
<? if(is_array($brandlist['list'])) { foreach($brandlist['list'] as $gkey => $gvar) { ?>
<li>
<a href="list.php?b=<?=$gvar['BrandID']?>&t=<?=$in['t']?>" target="_blank">
<? if($gvar['Logo']) { ?>
<img src="<?=RESOURCE_PATH?><? echo str_replace('thumb_', 'img_', $gvar['Logo']); ?>" class="conm" alt="<?=$gvar['BrandName']?>" />
<? } else { ?>
<img src="<?=CONF_PATH_IMG?>images/defaultbrand.gif" class="conm" alt="<?=$gvar['BrandName']?>" />
<? } ?>
</a>
<a style="display:block;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;" href="list.php?b=<?=$gvar['BrandID']?>&t=<?=$in['t']?>"><?=$gvar['BrandName']?></a>
<div class="online" style="font-size: 12px;position: relative">
<span style="display:block;width:70px;height:1px;margin:12px;background-color:#D5D5D5;overflow:hidden;"></span><p style="position: absolute;top: -12px;left: 85px;font-size: 10px">&nbsp;上架药品</p>
<div style="width:70px;float: left;height:1px;padding:0px;background-color:#D5D5D5;overflow:hidden;position: absolute;top:0px;left: 140px"></div>
</div>
<a style="display:block;" href="list.php?b=<?=$gvar['BrandID']?>&t=<?=$in['t']?>" class="numb"><?=$gvar['total']?>个</a>
</li>
<? } } ?>
</ul>
</div>
...
<form name="searchform" id="searchform" action="brand.php" method="get" style="display:inline;">
<input type="text" name="kw" class="search-input" id="search1" autocomplete="off" placeholder="输入商品品牌名称" />
