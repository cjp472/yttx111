...
//↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓文本记录↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓//begin tubo 增加日志接收记录 2015-11-09
$kLog = KLogger::instance(rtrim(LOG_PATH, '/') . '/alipay/');
$kLog->setDateFormat('Y-m-d G:i:s.u P');//年月日时分秒毫秒 时区
//转码
$logMsg = '页面通知：' . urldecode(http_build_query($in));
$kLog->logInfo($logMsg);
//↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑文本记录↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑//
    ...
    if($in['trade_status'] == 'TRADE_FINISHED' || $in['trade_status'] == 'TRADE_SUCCESS') {
		//判断该笔订单是否在商户网站中已经做过处理
	    	...
	    	if(empty($finfo['FinanceID'])){
				$sql_l  = "insert into ".$datacbase.".".DATATABLE."_order_finance(FinanceCompany,FinanceClient,FinanceOrder,FinanceAccounts,FinanceTotal,FinancePicture,FinanceAbout,FinanceToDate,FinanceUpDate,FinanceDate,FinanceUser,FinanceFlag,FinancePaysn,FinanceType,FinanceFrom) values(".$companyid.", ".$clientid.", '".$in['subject']."', ".$accountid.", '".$total_fee."', '', '".$in['body']."', '".date("Y-m-d")."', ".time().", ".time().",'-',2,'".$out_trade_no."','O','alipay')";       
				...
				//$clientinfo = $db->get_row("select ClientID,ClientCompanyName,ClientTrueName from ".DATABASEU.DATATABLE."_order_alipay where PaySN = '".$out_trade_no."' order by PayID desc limit 0,1");

				//$message = "【".$ucinfo['CompanySigned']."】经销商:".$clientinfo['ClientTrueName']."-".$clientinfo['ClientCompanyName']."于".date("Y-m-d")."通过支付宝支付一笔金额为:".$in['total_fee']."元的款项,请注意查收";
				//sms::get_setsms("3",$message);
			}
    }
    else {
      echo "trade_status=".$in['trade_status'];
    }
    ...
    <body>
        ...
        <table align="center" width="650" cellpadding="5" cellspacing="0" style="margin:30px auto;">
            ...
            <tr>
                ...
                <td class="font_content" align="left"><?php //echo $seller_email; ?>18003860209@189.cn</td>
            </tr>
            <tr>
                <td class="font_content" align="right">支付宝交易号：</td>
                <td class="font_content" align="left"><?php echo $in['trade_no']; ?></td>
            </tr>
            <tr>
                <td class="font_content" align="right">订单交易号：</td>
                <td class="font_content" align="left"><?php echo $in['out_trade_no']; ?></td>
            </tr>
            <tr>
                <td class="font_content" align="right">付款总金额：</td>
                <td class="font_content" align="left">¥ <?php echo $in['total_fee']; ?> 元</td>
            </tr>
            <tr>
                <td class="font_content" align="right">支付订单：</td>
                <td class="font_content" align="left"><?php if(empty($in['subject']) || $in['subject']=="0") echo '预付款'; else echo $in['subject']; ?></td>
            </tr>
            <tr>
                <td class="font_content" align="right">备注说明：</td>
                <td class="font_content" align="left"><?php echo $in['body']; ?></td>
            </tr>
            <tr>
                <td class="font_content" align="right">支付账号：</td>
                <td class="font_content" align="left"><?php echo $in['buyer_email']; ?></td>
            </tr>
            <tr>
                <td class="font_content" align="right">交易状态：</td>
                <td class="font_content" align="left"><?php if($in['trade_status']=="TRADE_SUCCESS" ||  $in['trade_status']=="TRADE_FINISHED") echo '支付成功'; else '支付未完成' ?></td>
            </tr>
        </table>
    </body>
</html>