...
//定位上一页
        function PrePage($isLink = false) {
                $prepage=$this->PageNum() - 1;
                $Previous = ($this->PageNum() >= 2) ? " <a href=\"?page=".$prepage.$this->Url($this->LinkAry)."\">上页</a> " : "<font color=#666666>上页</font> ";
                return $isLink ? ($this->PageNum() > 1 ? $isLink."?rt=etong&page=".$prepage.$this->Url($this->LinkAry) : '')  : $Previous;
//                 return $Previous;
        }
//定位下一页
        function NextPage($isLink = false) {
                ...
                return $isLink ? (($this->PageNum() <= $this->PageCount()-1) ? $isLink."?rt=etong&page=".$nextpage.$this->Url($this->LinkAry) : '') : $Next;
//                 return $Next;
        }
//定位最后一页
        function LastPage() {
...
//总条数
	...
	function multi($num, $perpage, $curpage, $mpurl, $maxpages = 0) 
	{
		...
		$multipage .= ($curpage < $pages ? '<a href="'.$mpurl.'page='.($curpage + 1).'">下一页 &#8250;</a>' : '').($curpage + $page - $offset <= $pages ? '<a href="'.$mpurl.'page='.$pages.'">最后一页 &#8250;&#8250;</a>' : '').($curpage == $maxpages ? '<a href="'.$mpurl.'pages='.$maxpages.'">&gt;</a>' : '').($pages > $page ? '<input type="text" name="custompage" onKeypress="if ((event.keyCode < 48 || event.keyCode > 57) && event.keyCode != 13) event.returnValue = false;"  onKeyDown="javascript: if(event.keyCode == 13) window.location=\''.$mpurl.'page=\'+this.value;">' : '');
