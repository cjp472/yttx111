...
class upfile{
	...
	* $over 复盖参数
	*/
	function upfile($pathend = "./", $format = "jpg|gif|png", $maxsize = 20000000, $over = 1)
	{
		$path = $pathend;
	...
	*/
	function upload($form, $rename = "")
	{
		...
		$rmsg   = $filear;
			...
			$this->ext = $this->getext($filear["name"]);//取得扩展名
	...
	*/
	function copyfile($filear){
		...
		if(!isset($filear["size"]) || $filear["size"] > $this->maxsize){
			return "上传文件 ".$filear["name"]." 大小超出系统限定值[2M]!";
		}
		...
		if(!@move_uploaded_file($filear["tmp_name"], $this->savepath.$this->savename))
		{
			...
			1=>"上传的文件超过了,系统中最大限制的值(2M)! ",
			2=>"上传的文件超过了,系统中最大限制的值(2M)! ",
			3=>"文件只有部分被上传! ",
			4=>"没有文件被上传! ");
